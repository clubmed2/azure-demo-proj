resource "azurerm_virtual_machine_scale_set" "user15vmss" {
name = "user15vmss"
...
ssh_keys {
    path = "/home/myadmin/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user15 등 
...
network_profile {
        ...
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user15-bpepool.id]
